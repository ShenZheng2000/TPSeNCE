...
# NOTE: try to incorporate QS-Attn => no use for now
# class PatchSampleF(nn.Module):
#     def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
#         # potential issues: currently, we use the same patch_ids for multiple images in the batch
#         super(PatchSampleF, self).__init__()
...
#     def forward(self, feats, num_patches=64, patch_ids=None, attn_mats=None, follow_seg=False):
#         return_ids = []
#         return_feats = []
#         return_mats = []
#         if self.use_mlp and not self.mlp_init:
...
#                         if follow_seg == True:
#                             patch_id = patch_ids[feat_id]
#                             mid = int(patch_id.shape[1] / 2)
#                             patch_id_old = patch_id[:, :mid]
#                             print("patch_id_old", patch_id_old.shape)
#                             patch_id_new = index[:, :num_patches][:, :mid]
#                             print("patch_id_new", patch_id_new.shape)
#                             patch_id = torch.hstack((patch_id_old, patch_id_new))
#                             # print("patch_id shape", patch_id.shape)
#                         else:
#                             patch_id = index[:, :num_patches] # [B, n_p]
...
#                     # NOTE: might comment out this one
#                     # patch_id = []
#             else:
#                 x_sample = feat_reshape
#                 patch_id = []
...
# NOTE: Trial PatchSampleF => No use for now
# class PatchSampleF(nn.Module):
#     def __init__(self, use_mlp=False, init_type='normal', init_gain=0.02, nc=256, gpu_ids=[]):
#         # potential issues: currently, we use the same patch_ids for multiple images in the batch
#         super(PatchSampleF, self).__init__()
...
#     def forward(self, feats, seg, num_patches=64, patch_ids=None):
#         return_ids = []
#         return_feats = []
#         return_segs = []
#         if self.use_mlp and not self.mlp_init:
...
#             B, H, W = feat.shape[0], feat.shape[2], feat.shape[3]
#             # seg: [bs,1, crop_size, crop_size]
#             seg = F.interpolate(seg,(H,W))
            
#             feat_reshape = feat.permute(0, 2, 3, 1).flatten(1, 2) # [B, HxW, C]
#             seg_reshape = seg.permute(0, 2, 3, 1).flatten(1, 2)  # [B,H*W, 1]
#             # reshape: same with feat_reshape
...
#                 # x_sample -> find the class in seg_reshape
#                 s_sample = seg_reshape[:,patch_id, :].flatten(0, 1)
...
#             if self.use_mlp:
#                 # NOTE: check skip_mlp here
#                 # if skip_mlp == False:
#                 mlp = getattr(self, 'mlp_%d' % feat_id)
#                 x_sample = mlp(x_sample)
                
#             return_ids.append(patch_id)
#             # NOTE: check skip_norm here
#             # if skip_norm == False:
#             x_sample = self.l2norm(x_sample)
...
#                 x_sample = x_sample.permute(0, 2, 1).reshape([B, x_sample.shape[-1], H, W])
#             return_feats.append(x_sample)
#             return_segs.append(s_sample)
#         return return_feats, return_ids, return_segs
